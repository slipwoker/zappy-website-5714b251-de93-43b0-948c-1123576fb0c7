...
const animatedElements = document.querySelectorAll('.feature-card, .testimonial-card, .program-card, .value-card, .team-member, .location-card');
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "שחייני דולפין משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('http://localhost:5001/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: '5714b251-de93-43b0-948c-1123576fb0c7',
                    name: data.name || '',
                    email: data.email || '',
